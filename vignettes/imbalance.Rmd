...
# Filtering
Once we have created synthetic examples, we should take care of which many of those instances are relevant to our problem. Filtering algorithms can be applied to *oversampled* datasets, to erase the least relevant instances.
...
$t_{-i}$ will be used to represent $(t_1, \ldots, t_{i-1}, t_{i+1}, \ldots, t_n)$ and similarly we can note $f_i(t_i, t_{-i})= f_i(t)$.

An strategic *Nash equilibrium* is a tuple $(t_1, \ldots t_n)$ where $f_i(t_i, t_{-i}) \ge f_i(t'_{i}, t_{-i})$ for every other $t'\in T$, and all $i=1, \ldots, n$. That is, a Nash strategy maximizes the payoff for all the players.

The strategy for each player will be picked with respect to a given probability $\delta_i \in \Delta_i = \{(\delta_i^{(1)}, \ldots, \delta_i^{(k_i)}) \in (R^{+})^k_i : \sum_{j=1}^{k_i} \delta_i^{(j)} = 1\}$. We define $\Delta_1 \times \ldots \times \Delta_n := \Delta$ and we call an element $\delta = (\delta_1, \ldots, \delta_n) \in \Delta$ strategy profile. Having a fixed strategy profile $\delta$, the overall payoff for the $i$-th player is defined as:
\[
  u_i(\delta) = \sum_{t\in T} \delta_i^{(t_i)} f_i(t)
\]

Given $u_i$ the payoff for a $\delta$ strategy profile in the $i$-th playter and $\delta\in \Delta$ we will denote
...
The payoff for a given instance is affected only by its own strategy and its $k$ nearest neighbors in $S\cup E$. That is, for every $x_i \in E$, we will have $u_i(\delta) = \sum_{j\in NN^k(x)} (x_i^T w_{ij} x_j)$ where $w_{ij} = g\left(d(x_i, x_j)\right)$, $g$ is a decresing function (the further, the lower payoff). In our implementation, we have considered $g(z) = \frac{1}{1+z^2}$, with $d$ the euclidean distance.
...
* Keep only those instances of `newSamples` with probability of being positive instances higher than $0.5$.
